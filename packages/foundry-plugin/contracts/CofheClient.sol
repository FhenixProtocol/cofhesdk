...
contract CofheClient is Test {
  ...
  function _createSingleEncryptedInput(
    ...
    (BatchedEncryptedInput[] memory inputs, bytes memory sig) = createEncryptedInputsBatch(utypes, values);
    ctHash = inputs[0].ctHash;
    signature = sig;
  }

  /// @notice Creates an encrypted boolean input as hash plus proof.
  function createInEbool(bool value) public returns (externalEbool hash, bytes memory proof) {
    (uint256 ctHash, bytes memory signature) = _createSingleEncryptedInput(Utils.EBOOL_TFHE, value ? 1 : 0);
    hash = externalEbool.wrap(bytes32(ctHash));
    proof = signature;
  }

  /// @notice Creates an encrypted uint8 input as hash plus proof.
  function createInEuint8(uint8 value) public returns (externalEuint8 hash, bytes memory proof) {
    (uint256 ctHash, bytes memory signature) = _createSingleEncryptedInput(Utils.EUINT8_TFHE, value);
    hash = externalEuint8.wrap(bytes32(ctHash));
    proof = signature;
  }

  /// @notice Creates an encrypted uint16 input as hash plus proof.
  function createInEuint16(uint16 value) public returns (externalEuint16 hash, bytes memory proof) {
    (uint256 ctHash, bytes memory signature) = _createSingleEncryptedInput(Utils.EUINT16_TFHE, value);
    hash = externalEuint16.wrap(bytes32(ctHash));
    proof = signature;
  }

  /// @notice Creates an encrypted uint32 input as hash plus proof.
  function createInEuint32(uint32 value) public returns (externalEuint32 hash, bytes memory proof) {
    (uint256 ctHash, bytes memory signature) = _createSingleEncryptedInput(Utils.EUINT32_TFHE, value);
    hash = externalEuint32.wrap(bytes32(ctHash));
    proof = signature;
  }

  /// @notice Creates an encrypted uint64 input as hash plus proof.
  function createInEuint64(uint64 value) public returns (externalEuint64 hash, bytes memory proof) {
    (uint256 ctHash, bytes memory signature) = _createSingleEncryptedInput(Utils.EUINT64_TFHE, value);
    hash = externalEuint64.wrap(bytes32(ctHash));
    proof = signature;
  }

  /// @notice Creates an encrypted uint128 input as hash plus proof.
  function createInEuint128(uint128 value) public returns (externalEuint128 hash, bytes memory proof) {
    (uint256 ctHash, bytes memory signature) = _createSingleEncryptedInput(Utils.EUINT128_TFHE, value);
    hash = externalEuint128.wrap(bytes32(ctHash));
    proof = signature;
  }

  /// @notice Creates an encrypted address input as hash plus proof.
  function createInEaddress(address value) public returns (externalEaddress hash, bytes memory proof) {
  ...
  // client - the single-item createInEuintX helpers above are thin wrappers over a batch of
  ...
  function createEuint32sBatch(
    ...
    (BatchedEncryptedInput[] memory inputs, bytes memory sig) = createEncryptedInputsBatch(utypes, rawValues);
