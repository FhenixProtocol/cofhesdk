...
import { ACP } from '@cofhe/mock-contracts/contracts/ACP.sol';
...
contract CofheClientTest is CofheTest {
  ...
  function testDecryptForTx_withPermit() public {
    ...
    ACP memory permit = cofheClient.permit_createSelf();
    bytes32 ctHash = euint32.unwrap(store.storedEuint32());
  ...
  function testPermitCreateShared_isValid() public view {
    ...
    ACP memory permit = cofheClient.permit_createShared(bob);
  ...
  function testPermitExportImportShared() public {
    ...
    ACP memory alicePermit = cofheClient.permit_createShared(bob);
    ...
    ACP memory bobPermit = bobClient.permit_importShared(exported);
