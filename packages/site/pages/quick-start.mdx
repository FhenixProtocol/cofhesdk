...
| `@cofhe/sdk`                      | `^0.4.0` | Client-side encryption, decryption, and ACP management                 |
...
Use `hre.cofhe.createClientWithBatteries` to get a fully configured SDK client with a self-ACP, then encrypt → send → decrypt.
...
2. `createClientWithBatteries` created an SDK client connected to the Hardhat network, with a self-ACP ready to go.
...
5. `decryptForView` used the ACP to decrypt the handle back to `42n` locally.
...
- [Decrypting to View](/sdk/decrypt-to-view) — reveal encrypted state in your UI with ACPs.
- [Decrypting to Transact](/sdk/decrypt-to-tx) — reveal values on-chain with a verifiable signature.
- [ACPs](/sdk/acp) — create, share, and manage decryption authorization.
