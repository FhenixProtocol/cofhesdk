...
- Off-chain: produced by `client.encryptInputs([...]).execute()` as a `[...hashes, signature]` tuple — one ciphertext hash per input, followed by a single signature authenticating the whole batch together.
