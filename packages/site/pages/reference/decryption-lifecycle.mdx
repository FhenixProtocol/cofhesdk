...
It is not about ACPs, return types, or builder methods. It is about what happens after you already have a `ctHash` and ask CoFHE to decrypt it.
...
The SDK also treats short-lived submit-time `404 Not Found` responses as retryable during an initial indexing window — but only while the backend's error body reports `ct_not_found` (the "ciphertext not indexed yet" case). A `404` carrying any other error code is surfaced immediately as that code's dedicated error, not retried. By default, the `ct_not_found` retry window is 10 seconds and can be changed via `.set404RetryTimeout(timeoutMs)` on either decrypt builder.
...
decrypt call starts
  -> submit request
    -> 404, error: ct_not_found, within retry window: SDK retries submit automatically
    -> 404, error: ct_not_found, after retry window: fail and surface submit-time error
    -> 404, any other error code: fail immediately, not retried
...
- If you get a submit-time `404` reporting `ct_not_found`, the SDK now retries automatically for a short window; increase `.set404RetryTimeout(...)` if your backend indexes handles more slowly. A `404` reporting anything else fails immediately.
...
Decrypt/sealoutput failures throw a typed `CofheError` (see [Migrating from cofhejs](/migrating-from-cofhejs) for the general `isCofheError`/`err.code` pattern). The threshold network's stable backend error codes each map to a dedicated `CofheErrorCode`, for example:

- `permit_denied`, `permit_expired`, `permit_invalid`, `permit_malformed` — ACP was rejected or malformed.
- `not_publicly_allowed` — no ACP was supplied and the handle isn't publicly decryptable.
- `ct_not_found` — the ciphertext handle is unknown to the ct-server (after any indexing retry window has elapsed).
- `unsupported_security_zone`, `unsupported_type` — the handle's zone or encryption type isn't supported.
- `internal_error`, `signing_failed`, `ct_source_error`, `permit_verifier_error`, `ct_source_timeout`, `permit_verifier_timeout` — backend-side failures.
- `permit_required`, `seal_failed` — sealoutput-specific.

`err.apiErrorCode` carries the raw backend string verbatim (e.g. `'permit_denied'`), independent of which `CofheErrorCode` it mapped to — useful for logging, or if the backend introduces a new code before the SDK adds a dedicated enum member for it (in that case `err.code` falls back to the operation's generic failure code, but `err.apiErrorCode` still tells you exactly what the backend reported).
