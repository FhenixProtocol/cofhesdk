...
| Decrypt for view | `client.decryptForView(ctHash, utype).execute()` | Requests decryption of a ciphertext handle via the Threshold Network using a permit. Returns the plaintext locally (not published on-chain). |
