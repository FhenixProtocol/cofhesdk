...
Both are pure functions of the receipt — no RPC calls — and pass raw `gasUsed` through unchanged on a real network. With `cofhe: { gasSummary: true }` in the config, a per-method table (raw vs adjusted gas) is printed after `hardhat test`. The table is reconstructed from chain history after the run, so transactions rolled back by `loadFixture`/`evm_revert` won't appear — use the receipt helpers for exact numbers. See [Mock Contracts → Gas reporting](/hardhat/mock-contracts#gas-reporting) for details.
