...
```bash
forge install FhenixProtocol/cofhesdk    # @cofhe/foundry-plugin + @cofhe/mock-contracts
```

Both packages live in the `cofhesdk` monorepo, so a single install covers them. Forge will check it out under `lib/`, so point the `@cofhe/foundry-plugin/` and `@cofhe/mock-contracts/` remappings at `lib/cofhesdk/packages/<package>/` instead of `node_modules/@cofhe/<package>/`.
