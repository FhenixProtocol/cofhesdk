...
#### Hash plus proof variants

`CofheClient` also provides `_asHashPlusProof` equivalents for each type. These return a split `(hash, proof)` pair that matches Solidity functions accepting `externalEuint*` + `bytes proof` instead of an `InE*` struct.

| Function                                    | Returns                            |
| ------------------------------------------- | ---------------------------------- |
| `createInEbool_asHashPlusProof(bool)`       | `(externalEbool, bytes memory)`    |
| `createInEuint8_asHashPlusProof(uint8)`     | `(externalEuint8, bytes memory)`   |
| `createInEuint16_asHashPlusProof(uint16)`   | `(externalEuint16, bytes memory)`  |
| `createInEuint32_asHashPlusProof(uint32)`   | `(externalEuint32, bytes memory)`  |
| `createInEuint64_asHashPlusProof(uint64)`   | `(externalEuint64, bytes memory)`  |
| `createInEuint128_asHashPlusProof(uint128)` | `(externalEuint128, bytes memory)` |
| `createInEaddress_asHashPlusProof(address)` | `(externalEaddress, bytes memory)` |
...
function testSetValueHashPlusProof(uint32 n) public {
    ...
    vm.prank(cofheClient.account());
    myContract.setValueHashPlusProof(hash, proof);
...
| `decryptForTx_withPermit(bytes32 ctHash, Permission permission)` | `(bytes32, uint256, bytes)` | Decrypts using an ACP. Same return shape.                                         |
...
| Function                                  | Returns              | Description                                                                                   |
| ----------------------------------------- | -------------------- | --------------------------------------------------------------------------------------------- |
| `permit_createSelf()`                     | `Permission`         | Creates a self-ACP signed by the connected account.                                           |
| `permit_createShared(address recipient)`  | `Permission`         | Creates the issuer half of a shared ACP.                                                      |
