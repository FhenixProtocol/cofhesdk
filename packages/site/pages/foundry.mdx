...
Mock FHE operations replicate the off-chain CoFHE work on-chain, which would normally inflate reported gas 2–3×. Under forge, `deployMocks()` excludes that mock-only work from gas metering by default (via `pauseGasMetering` cheatcodes), so `forge test --gas-report`, `forge snapshot`, and `gasleft()` measurements report numbers close to real-network costs — no configuration needed.

- Requires forge ≥ 1.0 (older versions had unreliable `pauseGasMetering` behavior). If cheatcodes are unavailable, the mocks silently fall back to normal metering — correctness is never affected.
- To see raw mock gas instead, opt out with `mockTaskManager.setMockGasExcluded(false)`.

:::note
Upgrading to a plugin version with this feature will lower existing `forge snapshot` numbers — the drop is the mock overhead disappearing from the report, not a change in your contracts.
:::
...
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `deployMocks()`                             | Deploys the complete CoFHE mock stack: `MockTaskManager`, `MockACL`, `MockZkVerifier`, `MockZkVerifierSigner`, `MockThresholdNetwork`, `MockThresholdNetworkSigner`. Mirrors the Hardhat plugin deployment order. Also enables mock-gas exclusion (see [Gas reporting](#gas-reporting)). |
...
| `decryptForView(bytes32 ctHash, Permission permission)`       | `uint256`                   | Seals and unseals with the ACP's sealing key for off-chain reading.               |

### ACPs

| Function                              | Returns           | Description                                                                                   |
| ------------------------------------- | ----------------- | --------------------------------------------------------------------------------------------- |
| `acp_createSelf()`                    | `Permission`      | Creates a self-ACP signed by the connected account.                                           |
| `acp_createShared(address recipient)` | `Permission`      | Creates the issuer half of a shared ACP.                                                      |
| `acp_exportShared(Permission)`        | `SharedACPExport` | Strips private fields for safe transmission.                                                  |
| `acp_importShared(SharedACPExport)`   | `Permission`      | Reconstructs a `Permission` as the recipient, adding the sealing key and recipient signature. |
