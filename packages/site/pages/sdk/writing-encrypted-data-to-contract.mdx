...
:::info
The helpers take the `bytes` immediately after the encrypted parameters as the signature slot, so
ERC-7984-style `*AndCall` overloads — where a `data` payload follows the proof — wire up without
any special handling. The one layout they reject is `external*` parameters that are not adjacent to
