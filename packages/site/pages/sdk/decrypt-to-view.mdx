...
2. Ensure you have an [ACP](/reference/dictionary#acp) that authorizes decryption of that value (see [ACP quickstart](#ACP-quickstart)).
3. Call `decryptForView(ctHash, utype)` and then run `.execute()` to get the plaintext (see [Decrypt for UI](#decrypt-for-ui)).

:::note
`decryptForView` always decrypts using an ACP (there is no `.withoutPermit()` mode).
...
3. Have an ACP available for the connected `chainId + account` (see [ACPs](./acp.mdx)).
...
await client.acp.getOrCreateSelfPermit();
```
...
`decryptForView(ctHash, utype)` returns a builder. Unlike `decryptForTx`, this flow always requires an ACP (there is no `.withoutPermit()` mode).
...
If you don’t call `.withPermit(...)`, `decryptForView` also uses the active ACP for the resolved `chainId + account`.
...
- **Wrong chain/account**: ACPs are scoped to `chainId + account`. If the user switches wallets or networks, create/select the correct ACP (or set them explicitly via `.setChainId(...)` / `.setAccount(...)`).
