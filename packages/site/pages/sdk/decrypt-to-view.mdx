...
:::note
For the shared production request states behind decryption, including submit-time `404`, submit-time `204` without `request_id`, and request polling after `request_id` creation, see [Decryption Lifecycle](./decryption-lifecycle.mdx).
:::
...
For the full meaning of submit retries versus request-status polling, see [Decryption Lifecycle](./decryption-lifecycle.mdx).
