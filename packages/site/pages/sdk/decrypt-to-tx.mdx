...
3. Determine whether the contract-level ACL policy for this specific handle requires an [ACP](/reference/dictionary#acp) (see [ACPs](./acp.mdx)).
   ...
   - If the policy allows **anyone** to decrypt it (a common setup), an ACP is not required and `.withoutACP()` will work.
...
declare const ctHash: `0x${string}`;
// ---cut---
const result = await client.decryptForTx(ctHash).withACP().execute(); // [!code focus]
