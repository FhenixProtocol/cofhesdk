...
3. Determine whether the contract-level ACL policy for this specific handle requires an [ACP](/reference/dictionary#acp) (see [ACPs](./acp.mdx)).

   - If the policy allows **anyone** to decrypt it (a common setup), an ACP is not required and `.withoutPermit()` will work.
...
In these cases, you can skip ACPs and use `.withoutPermit()`.
...
`decryptForTx(ctHash)` returns a builder. Before calling `.execute()`, you must select exactly one ACP mode: `.withPermit(...)` or `.withoutPermit()`.
...
const result = await client
  .decryptForTx(ctHash)
  .set404RetryTimeout(15_000) // [!code focus]
  .withPermit()
  .execute();
...
- `.withPermit()` uses the active ACP for the resolved `chainId + account`.
- `.withPermit(permitHash)` fetches a stored ACP by hash.
- `.withPermit(permit)` uses the provided ACP object.
...
- **ACP mode must be selected**: you must call exactly one of `.withPermit(...)` or `.withoutPermit()` before `.execute()`.
- **Wrong chain/account**: ACPs are scoped to `chainId + account`. If you use `.withPermit()` and get an ACL/ACP error, double-check you’re connected to the expected chain and account (or explicitly set them via `.setChainId(...)` / `.setAccount(...)`).
