...
3. Submit an on-chain transaction that either:
   ...
   - publishes the result via `FHE.publishDecryptResult(ctHash, plaintext, signature)`, or
   - verifies the result inside your contract via `FHE.verifyDecryptResult(ctHash, plaintext, signature)` (common for “unshield” style flows).
