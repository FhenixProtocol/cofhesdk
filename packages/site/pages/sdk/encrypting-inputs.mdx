...
2. Wrap each value with a typing helper (e.g. `Encryptable.uint32(…)`) to construct typed encryptable inputs.
3. Call `client.encryptInputs([...]).execute()` to produce `EncryptedItemInput` objects.
4. Submit a transaction to your contract and pass the encrypted inputs as `InE*` parameters.
