...
3. Call `client.encryptInputs([...]).execute()` to produce a [batch-verified result](#what-encryptinputs-returns) — one ciphertext hash per input, followed by a single shared signature.
4. Submit a transaction to your contract, passing the hashes and signature as `external*` + `bytes` parameters (see [Writing encrypted data to a contract](./writing-encrypted-data-to-contract.mdx)).
