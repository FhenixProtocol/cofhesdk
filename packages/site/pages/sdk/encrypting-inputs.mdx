...
| `Verify`    | Sends the proof to the CoFHE verifier's `/verifyBatch` endpoint. Returns the hashes and one shared batch signature. |

`Prove` normally dominates the total time. On the web it can run across multiple threads
— roughly 5× faster — if your app is served with the right headers. See
[Faster Encryption](/reference/faster-encryption).
