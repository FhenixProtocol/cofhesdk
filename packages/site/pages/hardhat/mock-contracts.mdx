...
:::warning
The summary is reconstructed from chain history after the run. Tests using `loadFixture` or manual `evm_snapshot`/`evm_revert` roll back their blocks, so those transactions won't appear in the table — use `getAdjustedGasUsed(receipt)` inside tests for exact per-transaction numbers. The summary is also best-effort: it may be incomplete after forced exits (`process.exit`, Ctrl+C).
:::
