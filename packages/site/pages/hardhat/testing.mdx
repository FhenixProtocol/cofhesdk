...
`createClientWithBatteries` pre-generates a self-ACP, so `decryptForView` and `decryptForTx().withPermit()` work immediately. For tests that need named ACPs or multiple signers, create them explicitly:
...
const permitHash = ACPUtils.getHash(permit);
cofheClient.acp.selectActivePermit(permitHash);
```
...
[`decryptForTx`](/sdk/decrypt-to-tx) returns a `{ ctHash, decryptedValue, signature }` tuple for on-chain submission. The ACP mode must be selected explicitly.
...
// @noErrors
import { CofheClient } from '@cofhe/sdk';
import { ACP } from '@cofhe/sdk/permits';
import { expect } from 'chai';
declare const cofheClient: CofheClient;
declare const ctHash: string;
declare const permit: ACP;
// ---cut---
const result = await cofheClient
  .decryptForTx(ctHash)
