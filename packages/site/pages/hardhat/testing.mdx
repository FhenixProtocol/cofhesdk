...
// ---cut---
// 1. Encrypt the input
const encrypted = await cofheClient.encryptInputs([Encryptable.uint32(100n)]).execute();
...
const decrypted = await cofheClient.decryptForView(ctHash, FheTypes.Uint32).execute();
...
// ---cut---
await myContract.revealValue(result.ctHash, result.decryptedValue, result.signature);
```
