...
| **ACP handling** | Automatic (uses most recent permit) | Explicit — `.withPermit()` / `.withPermit(permit)` / `.withPermit(hash)` / `.withoutPermit()` |
...
Permits are now called **ACPs** (Access Control Permissions) and are no longer auto-generated during initialization. All ACP operations are explicit through `client.acp`, with separate methods for self ACPs, sharing ACPs, and importing shared ACPs.
...
| **Creation** | `cofhejs.createPermit({ type, issuer })` | `client.acp.createSelf(...)`, `client.acp.createSharing(...)`, `client.acp.importShared(...)` |
| **Return type** | `Result<Permit>` | Direct `ACP` object |
| **Active ACP** | Implicitly used by `unseal` | `getOrCreateSelfPermit()` sets the active ACP; `decryptForView/Tx` uses it automatically |
| **Sharing** | `createPermit({ type: "sharing", recipient })` → `createPermit({ type: "recipient", ...permit })` | `createSharing(...)` → `importShared(...)` |
| **Selection** | Implicit | `client.acp.selectActivePermit(hash)` |
| **Retrieval** | `cofhejs.getPermit(...)` | `client.acp.getPermit(hash)`, `client.acp.getPermits()`, `client.acp.getActivePermit()` |
:::
::::
...
const permit2 = await client.acp.getOrCreateSelfPermit();
