...
| **ACP handling** | Automatic (uses most recent permit) | Explicit — `.withACP()` / `.withACP(permit)` / `.withACP(hash)` / `.withoutACP()` |
...
| **Retrieval** | `cofhejs.getPermit(...)` | `client.acp.getACP(hash)`, `client.acp.getACPs()`, `client.acp.getActiveACP()` |
:::
::::
...
const permit2 = await client.acp.getOrCreateSelfACP();
