...
`@cofhe/sdk` is the successor to `cofhejs`, redesigned around an explicit, builder-pattern API that gives you full control over encryption, decryption, and ACP (Access Control Permission) management. Migration should be strightforward with a 1-to-1 mapping of functions and features.
...
| **ACP handling** | Automatic (uses most recent acp) | Explicit — `.withACP()` / `.withACP(acp)` / `.withACP(hash)` / `.withoutACP()` |
...
ACPs are now called **ACPs** (Access Control Permissions) and are no longer auto-generated during initialization. All ACP operations are explicit through `client.acp`, with separate methods for self ACPs, sharing ACPs, and importing shared ACPs.

::::note
:::details[Changes]
| | `cofhejs` | `@cofhe/sdk` |
| ------------------- | ------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| **Auto-generation** | `generateACP: true` (default) | Never — always explicit |
| **Creation** | `cofhejs.createACP({ type, issuer })` | `client.acp.createSelf(...)`, `client.acp.createSharing(...)`, `client.acp.importShared(...)` |
| **Return type** | `Result<ACP>` | Direct `ACP` object |
| **Active ACP** | Implicitly used by `unseal` | `getOrCreateSelfACP()` sets the active ACP; `decryptForView/Tx` uses it automatically |
| **Sharing** | `createACP({ type: "sharing", recipient })` → `createACP({ type: "recipient", ...acp })` | `createSharing(...)` → `importShared(...)` |
| **Selection** | Implicit | `client.acp.selectActiveACP(hash)` |
| **Retrieval** | `cofhejs.getACP(...)` | `client.acp.getACP(hash)`, `client.acp.getACPs()`, `client.acp.getActiveACP()` |
:::
::::

### Before (`cofhejs`)

ACPs could be auto-generated during initialization (`generateACP: true`, the default), or created manually:
...
await cofhejs.initializeWithEthers({
  ...
  ethersSigner: signer,
  environment: 'TESTNET',
  // generateACP: true  ← default
});

// Manual creation
const result = await cofhejs.createACP({
  type: 'self',
...
ACPs are never auto-generated. You explicitly create, retrieve, and select them through `client.acp`:
...
// 4. Create acp & unseal
const acp = await cofhejs.createACP({
  type: 'self',
  issuer: wallet.address,
});
if (!acp.success) throw new Error(acp.error);
