...
3. Connect to client (`client.connect`)
   3b. Using adapters (`@cofhe/sdk/adapters`)
...
If you’re using another wallet/provider stack, see section 3b for `@cofhe/sdk/adapters`.
...
## 3b) Create viem clients with "@cofhe/sdk/adapters"
...
To switch the account connected to the cofhe client, use `cofheClient.connect()` with updated viem clients.
...
To manually disconnect use `cofheClient.disconnect()`.
