...
Encrypting against a **live chain** currently fails with `ZK_VERIFY_FAILED`: the CoFHE verifier's
batch endpoint is not deployed yet. Encryption works end-to-end against hardhat mocks. This is not
a problem with your migration, and it will start working with no further code changes once the
endpoint ships.
