...
const generate = function (dependency: string) {
  return ['// SPDX-License-Identifier: UNLICENSED', 'pragma solidity >0.0.0;', `import '${dependency}';`]
    .map((l) => `${l}\n`)
    .join('');
};
...
task('compile:silent', 'Compiles without printing warnings').setAction(async (args, hre, runSuper) => {
