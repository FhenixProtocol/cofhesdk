...
const MOCK_STUBS_DIR_NAME = 'cofhe-stubs';
const MOCK_STUBS_FILE_NAME = 'CofheMocksImports.sol';

/**
 * Returns the absolute paths of every .sol file shipped inside the
 * @cofhe/mock-contracts/contracts directory.
 */
function getMockContractsSolFiles(): string[] {
  let contractsDir: string;
  try {
    const pkgJson = require.resolve('@cofhe/mock-contracts/package.json');
...
/**
 * Generates a stub Solidity file that imports every contract from
 * @cofhe/mock-contracts/contracts.  Writing it to the Hardhat cache directory
 * (which is inside the project root) means:
 *   - localPathToSourceName() accepts it (no HH1007)
 *   - Hardhat resolves the npm-style "@cofhe/mock-contracts/..." imports
 *     the same way it resolves any other node_modules dependency
 *   - All mock contract artifacts end up in hre.artifacts, giving Hardhat
 *     full ABI knowledge so it can decode reverts and traces
 */
function generateMockContractsStubContent(): string {
  const imports = getMockContractsSolFiles()
    .map((f) => `import "@cofhe/mock-contracts/contracts/${path.basename(f)}";`)
    .join('\n');
  ...
  hre.cofhe = {
    ...
    mocks: {
      ...
      getMockACL: async () => {
        ...
        return hre.ethers.getContractAt(aclArtifact.abi, aclAddress) as unknown as MockACL;
      },
