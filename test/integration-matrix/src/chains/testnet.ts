...
function getChainPrivateKey(chainId: number): `0x${string}` {
  if (chainId === 420105) {
    return TEST_LOCALCOFHE_PRIVATE_KEY;
  }

  return (TEST_PRIVATE_KEY || DEFAULT_PRIVATE_KEY) as `0x${string}`;
}

export function isTestnetEnabled(chainId: number): boolean {
  const contractAddress = getSimpleTestAddress(chainId);
  if (!contractAddress) return false;
  if (getChainPrivateKey(chainId) === DEFAULT_PRIVATE_KEY) return false;
  return true;
}
